...
from process_engine_core.plugin import ERROR_PORT, MAIN_PORT, PluginContext, PluginResult
from process_engine_core.registry import PluginRegistry
...
__all__ = [
    ...
    "Engine",
    "RunControl",
    # Re-exported so an engine host has one import for "run this": the executor
    # and the checks it runs first. The definitions live in the core package,
    # which is the half the designer's API installs on its own.
    "DefinitionError",
    "combine_deliveries",
    "deliveries_from_run",
    "preview_result",
...
class Engine:
    def __init__(
        ...
        # A registry of specs belongs to the designer's API, which executes
        # nothing; handing one to an Engine is a wiring mistake, and saying so
        # here beats an AttributeError halfway into somebody's run.
        for key in registry.keys():
            registry.executable(key, require=True)
