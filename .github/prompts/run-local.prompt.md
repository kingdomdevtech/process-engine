...
Bring the app up locally and prove it works — both servers, then a real request through each.
Report the URLs and what you saw; don't stop at "started".
...
```powershell
.venv\Scripts\python.exe -m process_engine      # http://127.0.0.1:8000, docs at /docs
...
`designer/node_modules` is missing, install first: `pip install -e ".[dev,excel,mysql]"` and
...
Give the two URLs, the plugin and process counts, and anything that looked wrong. Flag whatever
you reused rather than started, and leave the servers running unless asked to stop them.
